...
export type TAction = 
    ...
    {type: 'new_day', payload: {message:'yay new day.'}} |
...
function reducer(state :IReducerState, action : TAction) :IReducerState{
    switch(action.type){
        case('added_goal'): {
            ...
            return {...state, current_goals: [...state.current_goals, action.payload] }
        }
        case('removed_goal'): {
            //console.log("Reducer: removed goal");
            return {...state, current_goals: [...state.current_goals.filter(goal => goal.id !== action.payload.id)] }
        ...
        case('edited_goal'): {
            ...
            return {
                ...state, 
                current_goals: [...state.current_goals.map(goal => {
        ...
        case('completed_goal'): {
            ...
            const newTotalExp = state.current_exp + state.current_goals.filter(goal => goal.id === action.payload.id)[0].rewarded_exp;
            ...
            if(didLevelUp){
                current_awards = [...state.current_awards.map(award =>{
            ...
            if(isSameDay(today, latestEntryDate)) {
                updatedHistory = [ ...state.history.map( (entry) => {
                    if(isSameDay(today, new Date(entry.date))){
                        
                        return {...entry, entries: [...entry.entries, {...state.current_goals.filter(goal => goal.id === action.payload.id)[0], completed: true, date_completed: new Date()} ] }
                    }
                    return entry;
                })]
            } else{
                updatedHistory = [...state.history,{date: new Date(),entries: [ {...state.current_goals.filter(goal => goal.id === action.payload.id)[0], completed: true, date_completed: new Date()} ]} ]
        ...
        case('added_award'): {
            ...
            return {...state, current_awards: [...state.current_awards, action.payload]}
        }
        case('claimed_award'): {
            //console.log('claimed_award');
            const foundReward = state.current_awards.find(award => award.id === action.payload.id); 
            if(!foundReward){
                throw new Error('no such reward found!');
            }
            return {...state,claimed_awards:[foundReward, ...state.claimed_awards], current_awards: [...state.current_awards.map(award => {
        ...
        case('edited_award'): {
            ...
            return {...state, current_awards: [...state.current_awards.map( award => {
                if(award.id === action.payload.id){
                    return action.payload;
                }
                return award;
        ...
        case('deleted_award'): {
            ...
            return {...state, current_awards: [...state.current_awards.filter( award => award.id !== action.payload.id)]}
        }
        case('deferred_award'): {
            //console.log('deferred_award');

            const deferred_award = [...state.current_awards.filter( award => award.id === action.payload.id)][0];
            return {...state, current_awards: [...state.current_awards.filter(award => award.id !== action.payload.id)], future_awards: [deferred_award, ...state.future_awards]}
        }
        case('new_day'):{
